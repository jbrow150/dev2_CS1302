...
import edu.westga.cs1302.task_tracker.model.AscendingByPriorityThenName;
...
public class MainWindow {
	...
	@FXML
	private ComboBox<TaskPriority> selectedPriorityCombo;
	...
	void selectTask(MouseEvent event) {
		...
		if (selectedTask != null) {
			...
			this.selectedPriorityCombo.setValue(selectedTask.getPriority());
		}
	}
	...
	private void sortTasks() {
		 Comparator<Task> comparator = this.order.getValue();
	        if (comparator != null) {
	            FXCollections.sort(this.taskList, comparator);
	        }
	}
	
	@FXML
	void updatePriority(ActionEvent event) {
	    Task selectedTask = this.tasks.getSelectionModel().getSelectedItem();
	    TaskPriority newPriority = this.selectedPriorityCombo.getValue();
	    if (selectedTask != null && newPriority != null) {
	        // Cannot change the name, only description and priority
	        // Create a new Task object because priority is final
	        Task updatedTask = new Task(selectedTask.getName(), selectedTask.getDescription(), newPriority);

	        // Replace old task with updated task in ObservableList
	        int index = this.taskList.indexOf(selectedTask);
	        this.taskList.set(index, updatedTask);

	        sortTasks(); // auto-sort after priority change
	    }
	}
	...
	public void initialize() {
        // Bind ListView to the backing ObservableList
        this.tasks.setItems(this.taskList);
        ...
        this.selectedPriorityCombo.getItems().addAll(TaskPriority.HIGH, TaskPriority.MEDIUM, TaskPriority.LOW);

        // Initialize order ComboBox
        ...
        this.order.getItems().add(new AscendingByPriorityThenName());
        this.order.getItems().add(new DescendingByPriorityThenName());
	}
}
